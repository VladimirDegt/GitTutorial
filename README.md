...
21. git log - показує історію (номер) комітів
22. git checkout <номер коміту> - перехід на потрібний коміт, робимо нову гілку та об'єднуємо
23. git config --list - виводе інфу по поточному git
24. git remote set-url origin повний шлях до нового створенного репозиторію - св'язує локальний репо з репо на гітхабі
25. nano .git/config - перевірка на який репозиторій веде url
